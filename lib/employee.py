# lib/employee.py
from __init__ import CURSOR, CONN
from department import Department


class Employee:
    """
    ORM mapping for the employees table.
    """

    # Cache of persisted objects {id: Employee(...)}
    all = {}

    def __init__(self, name, job_title, department_id, id=None):
        self.id = id
        self.name = name              # validated by @property
        self.job_title = job_title    # validated by @property
        self.department_id = department_id  # validated by @property

    def __repr__(self):
        return (
            f"<Employee {self.id}: {self.name}, {self.job_title}, "
            f"Department ID: {self.department_id}>"
        )

    # -------- properties (validation) --------
    @property
    def name(self):
        return self._name

    @name.setter
    def name(self, name):
        if isinstance(name, str) and len(name):
            self._name = name
        else:
            raise ValueError("Name must be a non-empty string")

    @property
    def job_title(self):
        return self._job_title

    @job_title.setter
    def job_title(self, job_title):
        if isinstance(job_title, str) and len(job_title):
            self._job_title = job_title
        else:
            raise ValueError("job_title must be a non-empty string")

    @property
    def department_id(self):
        return self._department_id

    @department_id.setter
    def department_id(self, department_id):
        if isinstance(department_id, int) and Department.find_by_id(department_id):
            self._department_id = department_id
        else:
            raise ValueError("department_id must reference a department in the database")

    # -------- schema management --------
    @classmethod
    def create_table(cls):
        """Create the employees table if it doesn't exist."""
        sql = """
            CREATE TABLE IF NOT EXISTS employees (
                id INTEGER PRIMARY KEY,
                name TEXT,
                job_title TEXT,
                department_id INTEGER,
                FOREIGN KEY (department_id) REFERENCES departments(id)
            )
        """
        CURSOR.execute(sql)
        CONN.commit()

    @classmethod
    def drop_table(cls):
        """Drop the employees table."""
        sql = "DROP TABLE IF EXISTS employees;"
        CURSOR.execute(sql)
        CONN.commit()

    # -------- persistence (CRUD) --------
    def save(self):
        """Insert a row for this Employee and cache it."""
        sql = "INSERT INTO employees (name, job_title, department_id) VALUES (?, ?, ?)"
        CURSOR.execute(sql, (self.name, self.job_title, self.department_id))
        CONN.commit()

        self.id = CURSOR.lastrowid
        type(self).all[self.id] = self

    def update(self):
        """Update this Employee's row in the DB."""
        sql = """
            UPDATE employees
            SET name = ?, job_title = ?, department_id = ?
            WHERE id = ?
        """
        CURSOR.execute(sql, (self.name, self.job_title, self.department_id, self.id))
        CONN.commit()

    def delete(self):
        """Delete this Employee's row and remove from cache."""
        sql = "DELETE FROM employees WHERE id = ?"
        CURSOR.execute(sql, (self.id,))
        CONN.commit()

        del type(self).all[self.id]
        self.id = None

    @classmethod
    def create(cls, name, job_title, department_id):
        """Initialize, save, and return a new Employee."""
        emp = cls(name, job_title, department_id)
        emp.save()
        return emp

    # -------- retrieval helpers --------
    @classmethod
    def instance_from_db(cls, row):
        """Return an Employee instance for the given row tuple (id, name, job_title, department_id)."""
        if not row:
            return None

        emp = cls.all.get(row[0])
        if emp:
            emp.name = row[1]
            emp.job_title = row[2]
            emp.department_id = row[3]
        else:
            emp = cls(row[1], row[2], row[3], id=row[0])
            cls.all[emp.id] = emp
        return emp

    @classmethod
    def get_all(cls):
        """Return a list of all Employee objects in the table."""
        rows = CURSOR.execute("SELECT * FROM employees").fetchall()
        return [cls.instance_from_db(row) for row in rows]

    @classmethod
    def find_by_id(cls, id):
        """Return the Employee with the given primary key (or None)."""
        row = CURSOR.execute("SELECT * FROM employees WHERE id = ?", (id,)).fetchone()
        return cls.instance_from_db(row)

    @classmethod
    def find_by_name(cls, name):
        """Return the first Employee with the given name (or None)."""
        row = CURSOR.execute("SELECT * FROM employees WHERE name = ?", (name,)).fetchone()
        return cls.instance_from_db(row)

    # -------- relationship (one-to-many) --------
    def reviews(self):
        """
        Return a list of Review objects that reference this employee via employee_id.
        Import inside method to avoid circular imports.
        """
        from review import Review
        CURSOR.execute("SELECT * FROM reviews WHERE employee_id = ?", (self.id,))
        rows = CURSOR.fetchall()
        return [Review.instance_from_db(row) for row in rows]